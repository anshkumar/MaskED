...
class Loss(object):
    ...
    def _loss_location(self, sigma=3.0):
        ...
        smoothl1loss = tf.keras.losses.Huber(
                                             delta=1., 
                                             reduction=\
        ...
        return [tf.math.divide_no_nan(tf.reduce_sum(loss_loc), num_pos) * \
    ...
    def _focal_conf_loss(self, alpha=0.25, gamma=1.5):
        ...
        alpha_factor = tf.where(
                                tf.keras.backend.equal(labels, 1), 
                                alpha_factor, 
                                1 - alpha_factor)
        ...
        loss = focal_weight * tf.keras.backend.binary_crossentropy(
                                                            labels, 
                                                            pred_cls, 
                                                            from_logits=False)
    ...
    def _loss_class(self):
        ...
        pos_indices = tf.where(self.conf_gt > 0 )
        num_pos = tf.shape(pos_indices)[0]
        return [tf.math.divide_no_nan(tf.reduce_sum(loss_conf), 
                                      tf.cast(num_pos, tf.float32)) * \
                                                        self._loss_weight_cls]
    ...
    def _loss_class_ohem(self):
        ...
        # Using tf.nn.softmax or 
        # tf.math.log(tf.math.reduce_sum(tf.math.exp(batch_conf), 1)) to might 
        # cause NaN problem. This is a known problem 
        ...
        # examples in a batch.
        # https://github.com/dbolya/yolact/blob/b97e82d809e5e69dc628930070a444 \ 
        # 42fd23617a/layers/modules/multibox_loss.py#L251
        # https://github.com/dbolya/yolact/blob/b97e82d809e5e69dc628930070a444 \
        # 42fd23617a/layers/box_utils.py#L316
        ...
        loss_c = tf.reshape(loss_c, (tf.shape(self.pred_cls)[0], -1))
        pos_indices = tf.where(self.conf_gt > 0 )
        loss_c = tf.tensor_scatter_nd_update(
                                            loss_c, 
                                            pos_indices, 
                                            tf.zeros(tf.shape(pos_indices)[0]))
        num_pos = tf.math.count_nonzero(
                                        tf.greater(self.conf_gt,0), 
                                        axis=1, 
                                        keepdims=True)
        num_neg = tf.clip_by_value(
                            num_pos * self._neg_pos_ratio, 
                            clip_value_min=tf.constant(
                                                       self._neg_pos_ratio, 
                                                       dtype=tf.int64), 
                            clip_value_max=tf.cast(
                                                   tf.shape(self.conf_gt)[1]-1, 
                                                   tf.int64))
        ...
        neg_indices = tf.where(
            (tf.cast(idx_rank, dtype=tf.int64) < num_neg) & (self.conf_gt == 0))
        ...
        target_labels = tf.one_hot(
                            tf.squeeze(target_labels), depth=self.num_classes)

        if tf.reduce_sum(
                        tf.cast(num_pos, tf.float32) + \
                        tf.cast(num_neg, tf.float32)) > 0.0:
            cce = tf.keras.losses.CategoricalCrossentropy(from_logits=False,
                reduction=tf.keras.losses.Reduction.NONE)
            loss_conf = tf.reduce_sum(
                                     cce(target_labels, target_logits)) / \
                        tf.reduce_sum(tf.cast(num_pos, tf.float32) + \
                                     tf.cast(num_neg, tf.float32))
        else:
            loss_conf = 0.0
    ...
    def _loss_mask(self):
        ...
        p_mask = tf.reshape(self.pred_mask, 
                    (-1, tf.shape(self.pred_mask)[2], 
                    tf.shape(self.pred_mask)[3], tf.shape(self.pred_mask)[4]))
        gt_mask = tf.reshape(self.masks, 
                        (-1, tf.shape(self.masks)[2], tf.shape(self.masks)[3]))
        ...
        _idx = tf.stack(
            (pos_classes, tf.range(tf.shape(pos_classes)[0], dtype=tf.int64)),
            axis=1)
        pos_p_masks = tf.gather_nd(pos_p_masks, _idx)

        #Resizing to the input size
        pos_p_masks = tf.expand_dims(pos_p_masks, axis=-1)
        pos_p_masks = tf.image.resize(
                        pos_p_masks, 
                        [self.config.MASK_SHAPE[0], self.config.MASK_SHAPE[1]], 
